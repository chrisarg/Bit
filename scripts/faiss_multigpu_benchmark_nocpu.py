...
def run_multi_gpu_benchmark(args):
    ...
    print("\t\t ... first 5 queries and references")
    for i in range(min(5, num_a)):
        print(f"Query {i}: 0x{h_queries[i]:016x}")
    for i in range(min(5, num_b)):
        print(f"Ref {i}: 0x{h_refs[i]:016x}")
    print()
