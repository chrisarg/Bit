...
my $report_path =
  File::Spec->catfile( $results_dir, "llm-summary-$run_tag.md" );

# Detect underlying architecture, vendor, and board model for proper PMU mapping
my $arch = '';
run [ 'uname', '-m' ], '>', \$arch;
chomp $arch;

my $vendor = 'unknown-vendor';
my $cpu    = 'unknown-cpu';
if ( open my $fh, '<', '/proc/cpuinfo' ) {
    while ( my $line = <$fh> ) {
        if ( $line =~ /^vendor_id\s*:\s*(.+)$/ ) {
            $vendor = $1;
        }
        if ( $line =~ /^model name\s*:\s*(.+)$/ ) {
            $cpu = $1;
...
# Dynamically assign performance monitoring strings based on hardware topology
my %perf_profiles;
if ( $arch eq 'aarch64' ) {
    if (   $board =~ /Rock64/i
        || $board =~ /RockPro64/i
...
'cycles,instructions,branches,branch-misses,cache-references,cache-misses',
            ...
            'cache-l2'      => 'cycles,instructions,LLC-loads,LLC-load-misses',
            'cache-l3-dram' => 'cycles,instructions',
            'cache-stalls'  => 'cycles,instructions',
            'buffers-pending' => 'cycles,instructions',
            'buffers-store'   => 'cycles,instructions',
            'execution-uops'  => 'cycles,instructions',
            'execution-ports' => 'cycles,instructions',
            'frontend'        => 'cycles,instructions',
            'frequency'       => 'cycles,instructions',
            'vectorization'   => 'cycles,instructions',
            'tlb'             =>
'cycles,instructions,dTLB-loads,dTLB-load-misses,iTLB-loads,iTLB-load-misses',
            'uncore-numa' => 'cycles,instructions',
            'power-rapl'  => 'cycles,instructions',
        );
    }
    else {
        %perf_profiles = (
...
'cycles,instructions,branches,branch-misses,cache-references,cache-misses',
            'cache-l1' =>
              'cycles,instructions,L1-dcache-loads,L1-dcache-load-misses',
            'cache-l2'      => 'cycles,instructions,l2d_cache,l2d_cache_refill',
            'cache-l3-dram' => 'cycles,instructions,LLC-loads,LLC-load-misses',
            'cache-stalls'  =>
              'cycles,instructions,stall_backend_cache,stall_backend_mem',
            'buffers-pending' => 'cycles,instructions,stall_backend_mem',
            'buffers-store'   => 'cycles,instructions,stall_backend_st',
            'execution-uops'  => 'cycles,instructions,op_spec,op_retired',
            'execution-ports' =>
              'cycles,instructions,stall_backend,stall_frontend',
            'frontend'      => 'cycles,instructions,stall_frontend',
            'frequency'     => 'cycles,instructions',
            'vectorization' => 'cycles,instructions,ase_spec,sve_spec',
            'tlb'           =>
'cycles,instructions,dTLB-loads,dTLB-load-misses,iTLB-loads,iTLB-load-misses',
            'uncore-numa' => 'cycles,instructions',
            'power-rapl'  => 'cycles,instructions',
...
elsif ( $arch eq 'x86_64' && $vendor eq 'AuthenticAMD' ) {
    %perf_profiles = (
        summary =>
'cycles,instructions,branches,branch-misses,cache-references,cache-misses',
        'cache-l1' =>
          'cycles,instructions,L1-dcache-loads,L1-dcache-load-misses',
        'cache-l2' =>
          'cycles,instructions,l2_request.l2_hw_pf,l2_request.l2_miss',
        'cache-l3-dram'   => 'cycles,instructions,LLC-loads,LLC-load-misses',
        'cache-stalls'    => 'cycles,instructions,de_dis_uop_queue_empty',
        'buffers-pending' => 'cycles,instructions,ls_dispatch.ld_stalls',
        'buffers-store'   => 'cycles,instructions,ls_dispatch.store_stalls',
        'execution-uops'  => 'cycles,instructions,ex_ret_instr,ex_ret_ops',
        'execution-ports' => 'cycles,instructions,fpu_pipe_assignment.total',
        'frontend'        =>
          'cycles,instructions,de_no_dispatch_per_slot.no_ops_from_frontend',
        'frequency'     => 'cycles,instructions,msr/aperf/,msr/mperf/',
        'vectorization' =>
          'cycles,instructions,fp_ret_ops.dble_ops_x87,fp_ret_ops.dble_ops_avx',
        'tlb' =>
'cycles,instructions,dTLB-loads,dTLB-load-misses,iTLB-loads,iTLB-load-misses',
        'uncore-numa' => 'cycles,instructions,data_fabric',
        'power-rapl'  =>
          'cycles,instructions,power/energy-pkg/,power/energy-ram/',
    );
}
elsif ( $arch eq 'x86_64' && $vendor eq 'GenuineIntel' ) {
    if (   $cpu =~ /Ultra/i
        || $cpu =~ /Alder Lake/i
        || $cpu =~ /Raptor Lake/i
        || $cpu =~ /Meteor Lake/i
        ...
        || $cpu =~ /Lunar Lake/i )
    {
        %perf_profiles = (
            summary =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/branches/,cpu_core/branch-misses/,cpu_core/cache-references/,cpu_core/cache-misses/',
            'cache-l1' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/mem_load_retired.l1_hit/,cpu_core/mem_load_retired.l1_miss/',
            'cache-l2' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/mem_load_retired.l2_hit/,cpu_core/mem_load_retired.l2_miss/',
            'cache-l3-dram' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/mem_load_retired.l3_hit/,cpu_core/mem_load_retired.l3_miss/',
            'cache-stalls' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/cycle_activity.stalls_mem_any/',
            'buffers-pending' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/l1d_pend_miss.pending/,cpu_core/l1d_pend_miss.pending_cycles/',
            'buffers-store' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/resource_stalls.sb/',
            'execution-uops' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/uops_issued.any/,cpu_core/uops_executed.thread/,cpu_core/uops_retired.retire_slots/',
            'execution-ports' => 'cpu_core/cycles/,cpu_core/instructions/',
            'frontend'        =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/idq_uops_not_delivered.core/',
            'frequency'     => 'cpu_core/cycles/,cpu_core/instructions/',
            'vectorization' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/fp_arith_inst_retired.scalar_double/,cpu_core/fp_arith_inst_retired.vector_128_double/,cpu_core/fp_arith_inst_retired.vector_256_double/',
            'tlb' =>
'cpu_core/cycles/,cpu_core/instructions/,cpu_core/dtlb_load_misses.walk_completed/',
            'uncore-numa' => 'cpu_core/cycles/,cpu_core/instructions/',
            'power-rapl'  =>
'cpu_core/cycles/,cpu_core/instructions/,power/energy-pkg/,power/energy-ram/',
        );
    }
    else {
        %perf_profiles = (
...
'cycles,instructions,branches,branch-misses,cache-references,cache-misses',
            'cache-l1' =>
'cycles,instructions,mem_load_retired.l1_hit,mem_load_retired.l1_miss',
            'cache-l2' =>
'cycles,instructions,mem_load_retired.l2_hit,mem_load_retired.l2_miss',
            'cache-l3-dram' => join(
                ',', qw(
            ...
            'vectorization' => join(
                ',', qw(
                  ...
                  fp_arith_inst_retired.vector_256_double
                  fp_arith_inst_retired.vector_512_double
                )
            ),
            'tlb' =>
'cycles,instructions,dTLB-loads,dTLB-load-misses,iTLB-loads,iTLB-load-misses',
            'uncore-numa' =>
              'cycles,instructions,unc_m_cas_count.rd,unc_m_cas_count.wr',
            'power-rapl' =>
              'cycles,instructions,power/energy-pkg/,power/energy-ram/',
        );
    }
...
my %profile_purpose = (
    summary =>
      'Timing ranking, IPC, branch behavior, and generic cache miss rate.',
    'cache-l1'      => 'Retired load L1 hit/miss behavior.',
    'cache-l2'      => 'Retired load L2 hit/miss behavior after the L1.',
    'cache-l3-dram' =>
      'Retired LLC hit/miss behavior and local DRAM demand-load misses.',
    'cache-stalls'    => 'Cycles stalled by L1D, L2, and L3 miss activity.',
    'buffers-pending' =>
      'L1 fill-buffer saturation and pending-miss occupancy.',
    'buffers-store' =>
'Store-buffer pressure, store-queue fullness, and outstanding data-read depth.',
    'execution-uops' =>
      'Issued, executed, and retired uops plus backend resource stalls.',
    'execution-ports' => 'One/two/three/four-port utilization distribution.',
    'frontend'        =>
      'Undelivered uops and cycles with no or at most one delivered uop.',
    'frequency' =>
      'APERF/MPERF ratio for frequency changes, including AVX-512 downclock.',
    'vectorization' =>
      'SIMD efficiency and packed vector instructions vs scalar fallbacks.',
    'tlb'         => 'Translation Lookaside Buffer loads and misses.',
    'uncore-numa' => 'Cross-socket, memory controller interconnect traffic.',
    'power-rapl'  => 'Package and RAM thermal/power metrics.',
);

my @perf_profile_names = csv_values(
    'PERF_PROFILES',
'summary,cache-l1,cache-l2,cache-l3-dram,cache-stalls,buffers-pending,buffers-store,execution-uops,execution-ports,frontend,frequency,vectorization,tlb,uncore-numa,power-rapl',
);
...
if ( $elevate ne 'never' ) {
    my $available = 0;
    eval {
        run [ 'sudo', '-n', 'true' ], '>', \my $out, '2>', \my $err;
        $available = 1 if $? == 0;
    };
    if ( $elevate eq 'always' && !$available ) {
        print
"ELEVATE=always: authenticating sudo for perf and scheduling priority...\n";
        die "Unable to authenticate sudo for ELEVATE=always.\n"
          unless system('sudo -v') == 0;

        eval {
            run [ 'sudo', '-n', 'true' ], '>', \my $out2, '2>', \my $err2;
            $available = 1 if $? == 0;
        };
        die
"sudo authentication did not create a usable noninteractive credential.\n"
          unless $available;
    }
    $sudo = 'sudo -n ' if $available;
}
warn
"Warning: running without sudo; perf or elevated priority may be unavailable.\n"
...
sub run_command {
    ...
    eval {
        $success =
          run [ 'sh', '-c', "cd " . shell_quote($root) . " && $command" ],
          '>', $log_path, '2>&1';
    };
    if ($@) {
        open my $fh, '>>', $log_path;
        print {$fh} "IPC::Run exception: $@\n";
        close $fh;
        return 0;
    }
    return $success;
...
sub parse_benchmark {
    ...
    while ( my $line = <$fh> ) {
        ...
        $values{gqps}    = $1
          if $line =~ /^average:.*\(([\d.]+)\s+Gqword-pairs\/s\)/;
...
my @columns =
  qw(tag build_status run_status libpopcnt cpu_tile k_block rows cols unroll buffer_size best_ns avg_ns gqps checksum perf_cycles perf_instructions perf_branches perf_branch_misses perf_cache_references perf_cache_misses);
...
for my $row (@results) {
    print {$csv}
      join( ',', map { defined $row->{$_} ? $row->{$_} : '' } @columns ), "\n";
}
close $csv;

my @valid = sort { number( $a->{avg_ns} ) <=> number( $b->{avg_ns} ) }
  grep {
         $_->{build_status} eq 'ok'
      && $_->{run_status} eq 'ok'
      && defined $_->{avg_ns}
  } @results;
...
print {$report} "- OpenMP places: `",
  ( $ENV{OMP_PLACES}    // 'runtime default' ), "`; thread binding: `",
  ( $ENV{OMP_PROC_BIND} // 'runtime default' ), "`\n";
print {$report} "- Priority: `$priority`; elevated execution: ",
  ( $sudo ? 'yes' : 'no' ), "\n";
print {$report}
...
for my $profile (@perf_profile_names) {
    print {$report}
"  - `$profile`: $profile_purpose{$profile} Events: `$perf_profiles{$profile}`\n";
}
...
for my $row (@valid) {
    ...
    printf {$report}
"| %d | %d | %s | %s | %dx%d | %s | %s | %.0f | %.3f | %.3f | %.3f | %.3f |\n",
      $rank, $row->{libpopcnt}, $row->{cpu_tile}, $row->{k_block},
      $row->{rows}, $row->{cols}, $row->{unroll}, $row->{buffer_size},
      number( $row->{avg_ns} ), number( $row->{gqps} ), $ipc, $cache_rate,
      $branch_rate;
}
print {$report} "\n## Failed configurations\n\n";
my @failed =
  grep { $_->{build_status} ne 'ok' || $_->{run_status} ne 'ok' } @results;
print {$report} "- None\n" unless @failed;
print {$report}
  "- `$_->{tag}`: build=$_->{build_status}, run=$_->{run_status}\n"
  for @failed;
